...
private:
    ...
    static std::expected<Dominoes, std::string> parse_dominoes(const nlohmann::json& dominoes_json);
    static std::expected<Zones, std::string>    parse_zones(const nlohmann::json& regions_json);
    static std::expected<Solution, std::string> parse_solution(const nlohmann::json& solution_json);
